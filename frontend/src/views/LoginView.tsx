...
export default function LoginView() {
  ...
  const handleLogin = async (formData: LoginForm) => {
    try {
      const { data } = await apiAxios.post(`/auth/login`, formData);
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(handleLogin)} className="bg-white px-5 py-20 rounded-lg space-y-10 mt-10" noValidate>
        ...
        <input type="submit" className="bg-cyan-400 p-3 text-lg w-full uppercase text-slate-600 rounded-lg font-bold cursor-pointer" value="Iniciar Sesión" />
      </form>
